...
event EventEmergencyWithdraw(
 ...
 function getClaimAble(
    ...
    returns(uint256)
  {
    return _amount - totalClaimed[_userAddr];
  }
  
  function showClaimBtn(address _userAddr, uint256 _amount) external view returns(bool){
      if (_amount - totalClaimed[_userAddr] > 0) {
          return true;
      }
      else {
          return false;
    ...
    external 
  {
    ...
    require(idoToken != address(0),'Invalid token');
    require(claimStartAt > 0 && block.timestamp > claimStartAt,'Claim has not started yet');
    require(_amount > 0,'Invalid amount');
    // already refunded
    require(userRefund[recipient] == false,'Refunded');

    uint256 claimAble = _amount - totalClaimed[recipient];
     if (thisBal > 0 && claimAble > 0) {
        isClaimed[recipient] = true;
        totalClaimed[recipient] = totalClaimed[recipient] + claimAble;
        IERC20(idoToken).safeTransfer(recipient, claimAble);
