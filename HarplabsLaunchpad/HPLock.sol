...
contract HPLock is IHPLock, Ownable, Pausable {
    ...
    // ID padding from HP v1, as there is a lack of a pausing mechanism
    ...
    Lock[] private _locks;
    mapping(address => EnumerableSet.UintSet) private _userLpLockIds; // user => lockId
    mapping(address => EnumerableSet.UintSet) private _userNormalLockIds; // user => lockId

    EnumerableSet.AddressSet private _lpLockedTokens; // set of normal tokens address
    EnumerableSet.AddressSet private _normalLockedTokens; //set of lp tokens address
    ...
    function unlock(uint256 lockId) external override whenNotPaused validLock(lockId) {
    ...
    function editLock(
        ...
        uint256 newAmount,
        uint256 newUnlockDate
    ) external override whenNotPaused validLock(lockId) {
        Lock storage userLock = _locks[_getActualIndex(lockId)];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
    ...
    function editLockDescription(uint256 lockId, string memory description)
        external whenNotPaused validLock(lockId)
    {
        Lock storage userLock = _locks[_getActualIndex(lockId)];
        require(
            userLock.owner == msg.sender,
