...
contract PrivateSale is Ownable, Pausable {
    ...
    function contribute(uint256 _amount) external payable whenNotPaused onlyRunning {
        ...
        if (privateSaleType == 1) {
            ...
            require(IHPERC20(holdingToken).balanceOf(user) >= holdingAmount, 'Insufficient holding');
        }
        ...
        if (currency == address(0)) {
            ...
            IHPERC20(currency).safeTransferFrom(_msgSender(), address(this), _amount);
        }
    ...
    function withdrawContribute() external whenNotPaused {
        ...
        if (currency == address(0)) {
            ...
            IHPERC20 currencyTokenErc20 = IHPERC20(currency);
